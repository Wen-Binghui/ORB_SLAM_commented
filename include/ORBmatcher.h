...
class ORBmatcher {
   public:
    ...
    int SearchByProjection(Frame& CurrentFrame, KeyFrame* pKF,
                           const std::set<MapPoint*>& sAlreadyFound, float th,
                           int ORBdist);
    ...
    // Search MapPoints tracked in Frame1 in Frame2 in a window centered at
    // their position in Frame1
    int WindowSearch(Frame& F1, Frame& F2, int windowSize,
                     std::vector<MapPoint*>& vpMapPointMatches2,
                     int minOctave = -1, int maxOctave = INT_MAX);
    // Refined matching when we have a guess of Frame 2 pose
    int SearchByProjection(Frame& F1, Frame& F2, int windowSize,
                           std::vector<MapPoint*>& vpMapPointMatches2);
    ...
    int Fuse(KeyFrame* pKF, std::vector<MapPoint*>& vpMapPoints,
             float th = 2.5);
