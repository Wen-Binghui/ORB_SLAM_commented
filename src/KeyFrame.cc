...
namespace ORB_SLAM
{
    ...
    KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB) : mnFrameId(F.mnId), mTimeStamp(F.mTimeStamp), mfGridElementWidthInv(F.mfGridElementWidthInv),
                                                                       ...
                                                                       mvInvLevelSigma2(F.mvInvLevelSigma2), mpMap(pMap)
    {
        ...
        for (int i = 0; i < mnGridCols; i++) // F.mGrid 复制到 mGrid
        {
