...
// std::map<ORB_SLAM::KeyFrame *, size_t> mObservations 中指定KeyFrame添加 idx
// (某KeyFrame第idx个特征)
...
void MapPoint::EraseObservation(KeyFrame* pKF) {
    bool bBad = false;
    {
        ...
        if (mObservations.count(pKF)) {
            ...
            if (mObservations.size() <= 2) bBad = true;
        }
    }
...
void MapPoint::UpdateNormalAndDepth() {
...

    {
        ...
        mfMaxDistance =
            scaleFactor * dist * pRefKF->GetScaleFactor(nLevels - 1 - level);
