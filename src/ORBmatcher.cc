...
int ORBmatcher::SearchByProjection(Frame& F,
    ...
    for (size_t iMP = 0; iMP < vpMapPoints.size(); iMP++) {
        ...
        if (vNearIndices.empty()) continue;
        ...
        for (vector<size_t>::iterator vit = vNearIndices.begin(),
                                      vend = vNearIndices.end();
             vit != vend; vit++) {
            size_t idx = *vit;

            if (F.mvpMapPoints[idx]) continue;
...
int ORBmatcher::SearchByBoW(KeyFrame* pKF, Frame& F,
    ...
    vpMapPointMatches =
        vector<MapPoint*>(F.mvpMapPoints.size(), static_cast<MapPoint*>(NULL));
    ...
    while (KFit != KFend && Fit != Fend) {
        if (KFit->first == Fit->first) {
            ...
            for (size_t iKF = 0, iendKF = vIndicesKF.size(); iKF < iendKF;
                 iKF++) {
                ...
                for (size_t iF = 0, iendF = vIndicesF.size(); iF < iendF;
                     iF++) {
...
int ORBmatcher::WindowSearch(Frame& F1, Frame& F2, int windowSize,
                             vector<MapPoint*>& vpMapPointMatches2,
                             int minScaleLevel, int maxScaleLevel) {
    int nmatches = 0;
    vpMapPointMatches2 =
        vector<MapPoint*>(F2.mvpMapPoints.size(), static_cast<MapPoint*>(NULL));
    ...
    for (size_t i1 = 0, iend1 = F1.mvpMapPoints.size(); i1 < iend1; i1++) {
        ...
        if (bMinLevel)
            if (level1 < minScaleLevel) continue;
        ...
        for (vector<size_t>::iterator vit = vIndices2.begin(),
            ...
            if (vpMapPointMatches2[i2]) continue;
...
int ORBmatcher::SearchByProjection(Frame& F1, Frame& F2, int windowSize,
                                   vector<MapPoint*>& vpMapPointMatches2) {
    vpMapPointMatches2 = F2.mvpMapPoints;
    set<MapPoint*> spMapPointsAlreadyFound(vpMapPointMatches2.begin(),
    ...
    for (size_t i1 = 0, iend1 = F1.mvpMapPoints.size(); i1 < iend1; i1++) {
        ...
        if (!pMP1) continue;
        if (pMP1->isBad() || spMapPointsAlreadyFound.count(pMP1)) continue;
        ...
        vector<size_t> vIndices2 =
            F2.GetFeaturesInArea(u2, v2, windowSize, level1, level1);
        ...
        if (static_cast<float>(bestDist) <=
                static_cast<float>(bestDist2) * mfNNratio &&
            bestDist <= TH_HIGH) {
            vpMapPointMatches2[bestIdx2] = pMP1;
            nmatches++;
        }
...
int ORBmatcher::SearchForInitialization(Frame& F1, Frame& F2,
    ...
    vector<int> rotHist[HISTO_LENGTH];  // vector 里放数组
    for (int i = 0; i < HISTO_LENGTH; i++)
        rotHist[i].reserve(500);  // 每个数组大小 500
    ...
    for (size_t i1 = 0, iend1 = F1.mvKeysUn.size(); i1 < iend1; i1++) {
        cv::KeyPoint kp1 = F1.mvKeysUn[i1];
        int level1 = kp1.octave;
        if (level1 > 0)  // 只要0层，对应论文中所说finest level
            continue;

        // 在范围内找 F2 的特征点 -> vIndices2: F2特征点ids
        vector<size_t> vIndices2 =
        ...
        cv::Mat d1 = F1.mDescriptors.row(i1);  // 取Descriptors of F1
        ...
        for (vector<size_t>::iterator vit = vIndices2.begin();
            ...
            cv::Mat d2 = F2.mDescriptors.row(i2);  // 取Descriptors of F2
        ...
        if (bestDist <= TH_LOW) {
            if (bestDist < (float)bestDist2 * mfNNratio) {
                ...
                if (mbCheckOrientation) {
                    float rot =
                        F1.mvKeysUn[i1].angle - F2.mvKeysUn[bestIdx2].angle;
                    if (rot < 0.0) rot += 360.0f;
                    int bin = round(rot * factor);
                    if (bin == HISTO_LENGTH) bin = 0;
                    ROS_ASSERT(bin >= 0 && bin < HISTO_LENGTH);
...
int ORBmatcher::SearchForTriangulation(
    ...
    vector<cv::KeyPoint>& vMatchedKeys1, vector<cv::KeyPoint>& vMatchedKeys2,
        ...
        if (f1it->first == f2it->first) {
            for (size_t i1 = 0, iend1 = f1it->second.size(); i1 < iend1; i1++) {
                ...
                for (size_t i2 = 0, iend2 = f2it->second.size(); i2 < iend2;
                    ...
                    if (dist > TH_LOW) continue;

                    vDistIndex.push_back(make_pair(dist, idx2));
                }

                if (vDistIndex.empty()) continue;
                ...
                for (size_t id = 0; id < vDistIndex.size(); id++) {
                    if (vDistIndex[id].first > DistTh) break;
...
int ORBmatcher::Fuse(KeyFrame* pKF, vector<MapPoint*>& vpMapPoints, float th) {
    ...
    for (size_t i = 0; i < vpMapPoints.size(); i++) {
        ...
        if (bestDist <= TH_LOW) {
            ...
            if (pMPinKF) {
                if (!pMPinKF->isBad()) pMP->Replace(pMPinKF);
            } else {
...
int ORBmatcher::Fuse(KeyFrame* pKF, cv::Mat Scw,
    ...
    for (size_t iMP = 0, iendMP = vpPoints.size(); iMP < iendMP; iMP++) {
        ...
        if (bestDist <= TH_LOW) {
            MapPoint* pMPinKF = pKF->GetMapPoint(bestIdx);
            if (pMPinKF) {
                if (!pMPinKF->isBad()) pMPinKF->Replace(pMP);
            } else {
                pMP->AddObservation(pKF, bestIdx);
                pKF->AddMapPoint(pMP, bestIdx);
            }
...
int ORBmatcher::SearchBySim3(KeyFrame* pKF1, KeyFrame* pKF2,
                             ...
                             const cv::Mat& R12, const cv::Mat& t12, float th) {
    ...
    for (int i1 = 0; i1 < N1; i1++) {
        ...
        vector<float>::iterator it =
            lower_bound(vfScaleFactors2.begin(), vfScaleFactors2.end(), ratio);
        const int nPredictedLevel =
            min(static_cast<int>(it - vfScaleFactors2.begin()), nMaxLevel2);
    ...
    for (int i2 = 0; i2 < N2; i2++) {
        ...
        vector<float>::iterator it =
            lower_bound(vfScaleFactors1.begin(), vfScaleFactors1.end(), ratio);
        const int nPredictedLevel =
            min(static_cast<int>(it - vfScaleFactors1.begin()), nMaxLevel1);
    ...
    for (size_t i = 0, iend = LastFrame.mvpMapPoints.size(); i < iend; i++) {
        ...
        if (pMP) {
            if (!LastFrame.mvbOutlier[i]) {
                ...
                float radius =
                    th * CurrentFrame.mvScaleFactors[nPredictedOctave];

                vector<size_t> vIndices2 = CurrentFrame.GetFeaturesInArea(
                    u, v, radius, nPredictedOctave - 1, nPredictedOctave + 1);
    ...
    for (size_t i = 0, iend = vpMPs.size(); i < iend; i++) {
        ...
        if (pMP) {
            if (!pMP->isBad() && !sAlreadyFound.count(pMP)) {
                ...
                vector<float>::iterator it =
                    lower_bound(CurrentFrame.mvScaleFactors.begin(),
                                CurrentFrame.mvScaleFactors.end(), ratio);
                const int nPredictedLevel = min(
                    static_cast<int>(it - CurrentFrame.mvScaleFactors.begin()),
                    CurrentFrame.mnScaleLevels - 1);

                // Search in a window
                float radius =
                    th * CurrentFrame.mvScaleFactors[nPredictedLevel];
                ...
                if (bestDist <= ORBdist) {
                    ...
                    if (mbCheckOrientation) {
                        float rot = pKF->GetKeyPointUn(i).angle -
                                    CurrentFrame.mvKeysUn[bestIdx2].angle;
                        if (rot < 0.0) rot += 360.0f;
                        int bin = round(rot * factor);
                        if (bin == HISTO_LENGTH) bin = 0;
