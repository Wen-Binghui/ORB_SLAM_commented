...
Tracking::Tracking(ORBVocabulary* pVoc, FramePublisher* pFramePublisher,
                   MapPublisher* pMapPublisher, Map* pMap,
    ...
    mpORBextractor =
        new ORBextractor(nFeatures, fScaleFactor, nLevels, Score, fastTh);
    ...
    if (mbMotionModel) {
        ...
        cout << endl
             << "Motion Model: Disabled (not recommended, change settings "
                "UseMotionModel: 1)"
             << endl
             << endl;
    ...
    mTfBr.sendTransform(tf::StampedTransform(
        tfT, ros::Time::now(), "/ORB_SLAM/World", "/ORB_SLAM/Camera"));
}
...
void Tracking::Run() {
    ...
    // 接收图片输入 并执行 GrabImage
    ros::Subscriber sub = nodeHandler.subscribe(
        "/camera/image_raw", 1, &Tracking::GrabImage, this);  //! queue_size=1

    ros::spin();
}

// * Main Workflow of Tracking Thread
void Tracking::GrabImage(const sensor_msgs::ImageConstPtr& msg) {
    cv::Mat im;  // for grayscale image

    // # 1. Copy the ros image message to cv::Mat. Convert to grayscale if it is
    // a color image.
    ...
    // # 2. start tracking
    /*
        * mState 最初是 NO_IMAGES_YET == NOT_INITIALIZED
        读取图片，并开始提取特征点（Frame建立就提取）
    */

    if (mState == WORKING || mState == LOST)
        mCurrentFrame = Frame(im, cv_ptr->header.stamp.toSec(), mpORBextractor,
                              mpORBVocabulary, mK, mDistCoef);
    else
    /*
        * mpIniORBextractor = new ORBextractor(nFeatures * 2, 1.2, 8, Score,
       fastTh); (int nfeatures = 1000, float scaleFactor = (1.200000048F), int
       nlevels = 8, int scoreType = 1, int fastTh = 20)
    */
    {
        mCurrentFrame =
    ...
    if (mState == NO_IMAGES_YET) {
        mState = NOT_INITIALIZED;  // so NO_IMAGES_YET == NOT_INITIALIZED
    }

    mLastProcessedState = mState;

    if (mState == NOT_INITIALIZED) {
        FirstInitialization();  // and change mState = INITIALIZING;
        ...
        // Initial Camera Pose Estimation from Previous Frame (Motion Model or
        // Coarse) or Relocalisation
        if (mState == WORKING && !RelocalisationRequested()) {
            if (!mbMotionModel || mpMap->KeyFramesInMap() < 4 ||
                mVelocity.empty() ||
                mCurrentFrame.mnId < mnLastRelocFrameId + 2)
                bOK = TrackPreviousFrame();
            else {
                bOK = TrackWithMotionModel();
                if (!bOK) bOK = TrackPreviousFrame();
            }
        } else {
            bOK = Relocalisation();
        }
        ...
        if (mbMotionModel) {
            if (bOK && !mLastFrame.mTcw.empty()) {
                cv::Mat LastRwc =
                    mLastFrame.mTcw.rowRange(0, 3).colRange(0, 3).t();
                cv::Mat Lasttwc =
                    -LastRwc * mLastFrame.mTcw.rowRange(0, 3).col(3);
    ...
    if (!mCurrentFrame.mTcw.empty()) {
        ...
        mTfBr.sendTransform(tf::StampedTransform(
            tfTcw, ros::Time::now(), "ORB_SLAM/World", "ORB_SLAM/Camera"));
    }
}
...
void Tracking::FirstInitialization() {
    ...
    if (mCurrentFrame.mvKeys.size() > 100)  // * keypoints num 最少要100个
    {
        mInitialFrame = Frame(mCurrentFrame);  // copy Frame
        mLastFrame = Frame(mCurrentFrame);
        mvbPrevMatched.resize(mCurrentFrame.mvKeysUn.size());  // vector of P2f
...
void Tracking::Initialize() {
    // Check if current frame has enough keypoints, otherwise reset
    // initialization process
    if (mCurrentFrame.mvKeys.size() <= 100) {
        fill(mvIniMatches.begin(), mvIniMatches.end(),
             -1);  // fill the mvIniMatches with -1
    ...
    if (nmatches < 100)  // * 如果两幅图matches数目不够, 回到NOT_INITIALIZED状态
                         // (之后mInitialFrame会更新)
    {
        mState = NOT_INITIALIZED;
        return;
...
void Tracking::CreateInitialMap(cv::Mat& Rcw, cv::Mat& tcw) {
    ...
    // ! Insert KFs in the map (The first two matches)
...
bool Tracking::TrackPreviousFrame() {
    ...
    // Search first points at coarse scale levels to get a rough initial
    // estimate
    int minOctave = 0;
    int maxOctave = mCurrentFrame.mvScaleFactors.size() - 1;
    if (mpMap->KeyFramesInMap() > 5) minOctave = maxOctave / 2 + 1;

    int nmatches = matcher.WindowSearch(mLastFrame, mCurrentFrame, 200,
                                        vpMapPointMatches, minOctave);

    // If not enough matches, search again without scale constraint
    if (nmatches < 10) {
        nmatches = matcher.WindowSearch(mLastFrame, mCurrentFrame, 100,
                                        vpMapPointMatches, 0);
        if (nmatches < 10) {
            vpMapPointMatches =
                vector<MapPoint*>(mCurrentFrame.mvpMapPoints.size(),
    ...
    if (nmatches >= 10) {
        ...
        nmatches += matcher.SearchByProjection(mLastFrame, mCurrentFrame, 15,
                                               vpMapPointMatches);
...
bool Tracking::TrackLocalMap() {
    // Tracking from previous frame or relocalisation was succesfull and we have
    // an estimation of the camera pose and some map points tracked in the
    // frame. Update Local Map and Track
...
bool Tracking::NeedNewKeyFrame() {
    ...
    // Not insert keyframes if not enough frames from last relocalisation have
    // passed
    if (mCurrentFrame.mnId < mnLastRelocFrameId + mMaxFrames &&
        mpMap->KeyFramesInMap() > mMaxFrames)
        return false;
    ...
    const bool c1b = mCurrentFrame.mnId >= mnLastKeyFrameId + mMinFrames &&
                     bLocalMappingIdle;
    // Condition 2: Less than 90% of points than reference keyframe and enough
    // inliers
    const bool c2 =
        mnMatchesInliers < nRefMatches * 0.9 && mnMatchesInliers > 15;

    if ((c1a || c1b) && c2) {
        // If the mapping accepts keyframes insert, otherwise send a signal to
        // interrupt BA, but not insert yet
...
void Tracking::UpdateReferencePoints() {
    ...
    for (vector<KeyFrame*>::iterator itKF = mvpLocalKeyFrames.begin(),
                                     itEndKF = mvpLocalKeyFrames.end();
         itKF != itEndKF; itKF++) {
        KeyFrame* pKF = *itKF;
        vector<MapPoint*> vpMPs = pKF->GetMapPointMatches();

        for (vector<MapPoint*>::iterator itMP = vpMPs.begin(),
                                         itEndMP = vpMPs.end();
             itMP != itEndMP; itMP++) {
            MapPoint* pMP = *itMP;
            if (!pMP) continue;
...
void Tracking::UpdateReferenceKeyFrames() {
    ...
    for (size_t i = 0, iend = mCurrentFrame.mvpMapPoints.size(); i < iend;
         i++) {
        if (mCurrentFrame.mvpMapPoints[i]) {
    ...
    for (map<KeyFrame*, int>::iterator it = keyframeCounter.begin(),
    ...
    for (vector<KeyFrame*>::iterator itKF = mvpLocalKeyFrames.begin(),
        ...
        for (vector<KeyFrame*>::iterator itNeighKF = vNeighs.begin(),
...
bool Tracking::Relocalisation() {
    ...
    // Relocalisation is performed when tracking is lost and forced at some
    // stages during loop closing Track Lost: Query KeyFrame Database for
    // keyframe candidates for relocalisation
    vector<KeyFrame*> vpCandidateKFs;
    if (!RelocalisationRequested())
        vpCandidateKFs =
            mpKeyFrameDB->DetectRelocalisationCandidates(&mCurrentFrame);
    else  // Forced Relocalisation: Relocate against local window around last
          // keyframe
    {
    ...
    while (nCandidates > 0 && !bMatch) {
        for (size_t i = 0; i < vpCandidateKFs.size(); i++) {
            ...
            if (!Tcw.empty()) {
                ...
                for (size_t io = 0, ioend = mCurrentFrame.mvbOutlier.size();
                     io < ioend; io++)
                ...
                if (nGood < 50) {
                    ...
                    if (nadditional + nGood >= 50) {
                        ...
                        if (nGood > 30 && nGood < 50) {
                            ...
                            for (size_t
                                     ip = 0,
                                     ipend = mCurrentFrame.mvpMapPoints.size();
                                 ip < ipend; ip++)
                            ...
                            if (nGood + nadditional >= 50) {
                                ...
                                for (size_t io = 0;
                                     io < mCurrentFrame.mvbOutlier.size(); io++)
                                    if (mCurrentFrame.mvbOutlier[io])
                                        mCurrentFrame.mvpMapPoints[io] = NULL;
                            }
                        }
